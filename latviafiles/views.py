...
from .forms import (DEFAULT_LATITUDE, DEFAULT_LONGITUDE, DEFAULT_CENTER,
                    ...
                    MapSettingsForm, UpdateMapPermissionsForm, LatvianCaseForm)
from .models import DataLayer, Licence, Map, Pictogram, TileLayer, TileLayerWMS, WMSCategory, WMSProvider, LatviaCS
...
class PublicMaps(TemplateView, PaginatorMixin): # PUBLIC MAPS
...
class MapDetailMixin:
    ...
    def get_context_data(self, **kwargs):
        ...
        properties = {
            ...
            'urls': _urls_for_js(),
            'latvianCaseLayers': LatviaCS.get_list(),
...
def LatvianCaseView(request):
    form = LatvianCaseForm(request.POST or None)
    sust_value = 0
    fulu_value = 0
    fuve_value = 0
    mytr_value= 0
    ...
    if form.is_valid():
        ...
        sust_value = form.cleaned_data["sust"]
        mytr_value = form.cleaned_data["mytr"]
        fuve_value = form.cleaned_data["fuve"]
        fulu_value = form.cleaned_data["fulu"]
        annal_value = form.cleaned_data["annal"]
        sust_check = form.cleaned_data["sustCheck"]
        mytr_check = form.cleaned_data["mytrCheck"]
        fuve_check = form.cleaned_data["fuveCheck"]
        fulu_check = form.cleaned_data["fuluCheck"]
        annal_check = form.cleaned_data["annalCheck"]
        method_value = form.cleaned_data["method"]

        source = "/srv/umap/root/data"
        inpath = source + '/latvian_case/latvia_data.shp'

        ilsust = source + '/latvian_case/interpolated_sust.tif'
        sust_data = rasterio.open(ilsust, dtype='float64')
        array_sust = sust_data.read(1)
        if sust_value > 0:
            array_sust[(array_sust < sust_value)] = 0
            array_sust[(array_sust >= sust_value)] = 1
        else:
            array_sust[(array_sust <= 0)] = 0
            array_sust[(array_sust > 0)] = 1

        ilmytr = source + '/latvian_case/interpolated_mytr.tif'
        mytr_data = rasterio.open(ilmytr, dtype='float64')
        array_mytr = mytr_data.read(1)
        if mytr_value > 0:
            array_mytr[(array_mytr < mytr_value)] = 0
            array_mytr[(array_mytr >= mytr_value)] = 1
        else:
            array_mytr[(array_mytr <= 0)] = 0
            array_mytr[(array_mytr > 0)] = 1

        ilfuve = source + '/latvian_case/interpolated_fuve.tif'
        fuve_data = rasterio.open(ilfuve, dtype='float64')
        array_fuve = fuve_data.read(1)
        if fuve_value > 0:
            array_fuve[(array_fuve < fuve_value)] = 0
            array_fuve[(array_fuve >= fuve_value)] = 1
        else:
            array_fuve[(array_fuve <= 0)] = 0
            array_fuve[(array_fuve > 0)] = 1

        ilfulu = source + '/latvian_case/interpolated_fulu.tif'
        fulu_data = rasterio.open(ilfulu, dtype='float64')
        array_fulu = fulu_data.read(1)
        if fulu_value > 0:
            array_fulu[(array_fulu < fulu_value)] = 0
            array_fulu[(array_fulu >= fulu_value)] = 1
        else:
            array_fulu[(array_fulu <= 0)] = 0
            array_fulu[(array_fulu > 0)] = 1

        ilannal = source + '/latvian_case/interpolated_annal.tif'
        annal_data = rasterio.open(ilannal, dtype='float64')
        array_annal = annal_data.read(1)
        if annal_value > 0:
            array_annal[(array_annal < annal_value)] = 0
            array_annal[(array_annal >= annal_value)] = 1
        else:
            array_annal[(array_annal <= 0)] = 0
            array_annal[(array_annal > 0)] = 1

        includedLayers = []
        if sust_check: 
            includedLayers.append(array_sust)
        if mytr_check: 
            includedLayers.append(array_mytr)
        if fuve_check: 
            includedLayers.append(array_fuve)
        if fulu_check: 
            includedLayers.append(array_fulu)
        if annal_check: 
            includedLayers.append(array_annal)

        if (method_value == 0):
            i = 0
            while i < len(includedLayers):
                if i == 0:
                    array_total = includedLayers[i]
                else:
                    array_total = np.multiply(array_total, includedLayers[i])
                i = i + 1
        else:
            r = 0
            while r < len(includedLayers):
                if r == 0:
                    array_total = includedLayers[r]
                else:
                    array_total = np.add(array_total, includedLayers[r])
                r = r + 1
            array_total[(array_total >= 1)] = 1

        driver = gdal.GetDriverByName('GTiff')
        driver2 = gdal.GetDriverByName('PNG')

        src_dataset = gdal.Open(ilsust)
        geotransform = src_dataset.GetGeoTransform()
        spatialreference = src_dataset.GetProjection()
        ncol = src_dataset.RasterXSize
        nrow = src_dataset.RasterYSize
        minx = geotransform[0]
        maxx = geotransform[0] + ncol * geotransform[1] + nrow * geotransform[2]

        miny = geotransform[3] + ncol * geotransform[4] + nrow * geotransform[5]
        maxy = geotransform[3]

        dst_sust = driver.Create(settings.MEDIA_ROOT + '/latvian_case/sust' + str(sust_value) + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_sust.SetGeoTransform(geotransform)
        dst_sust.SetProjection(spatialreference)
        dst_sust.GetRasterBand(1).SetNoDataValue(0)
        """ array_sust = np.flipud(array_sust) """
        dst_sust.GetRasterBand(1).WriteArray(array_sust)
        ds2s = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/sust' + str(sust_value) + '.png', dst_sust, 0)
        dst_sust = None

        dst_mytr = driver.Create(settings.MEDIA_ROOT + '/latvian_case/mytr' + str(mytr_value) + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_mytr.SetGeoTransform(geotransform)
        dst_mytr.SetProjection(spatialreference)
        dst_mytr.GetRasterBand(1).SetNoDataValue(0)
        dst_mytr.GetRasterBand(1).WriteArray(array_mytr)
        ds2m = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/mytr' + str(mytr_value) + '.png', dst_mytr, 0)
        dst_mytr = None

        dst_fuve = driver.Create(settings.MEDIA_ROOT + '/latvian_case/fuve' + str(fuve_value) + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_fuve.SetGeoTransform(geotransform)
        dst_fuve.SetProjection(spatialreference)
        dst_fuve.GetRasterBand(1).SetNoDataValue(0)
        dst_fuve.GetRasterBand(1).WriteArray(array_fuve)
        ds2fv = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/fuve' + str(fuve_value) + '.png', dst_fuve, 0)
        dst_fuve = None

        dst_fulu = driver.Create(settings.MEDIA_ROOT + '/latvian_case/fulu' + str(fulu_value) + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_fulu.SetGeoTransform(geotransform)
        dst_fulu.SetProjection(spatialreference)
        dst_fulu.GetRasterBand(1).SetNoDataValue(0)
        dst_fulu.GetRasterBand(1).WriteArray(array_fulu)
        ds2fl = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/fulu' + str(fulu_value) + '.png', dst_fulu, 0)
        dst_fulu = None

        dst_annal = driver.Create(settings.MEDIA_ROOT + '/latvian_case/annal' + str(annal_value) + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_annal.SetGeoTransform(geotransform)
        dst_annal.SetProjection(spatialreference)
        dst_annal.GetRasterBand(1).SetNoDataValue(0)
        dst_annal.GetRasterBand(1).WriteArray(array_annal)
        ds2an = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/annal' + str(annal_value) + '.png', dst_annal, 0)
        dst_annal = None

        dst_total = driver.Create(settings.MEDIA_ROOT + '/latvian_case/total' + 'annal' + str(annal_value) + 'fulu' + str(fulu_value) + 'fuve' + str(fuve_value) + 'mytr' + str(mytr_value) + 'sust' + str(sust_value) + 'method' + str(method_value) + 'temp' + '.tif', ncol, nrow, 1, gdal.GDT_Int16)
        dst_total.SetGeoTransform(geotransform)
        dst_total.SetProjection(spatialreference)
        dst_total.GetRasterBand(1).SetNoDataValue(0)
        dst_total.GetRasterBand(1).WriteArray(array_total)
        ds2to = driver2.CreateCopy(settings.MEDIA_ROOT + '/latvian_case/total' + 'annal' + str(annal_value) + 'fulu' + str(fulu_value) + 'fuve' + str(fuve_value) + 'mytr' + str(mytr_value) + 'sust' + str(sust_value) + 'method' + str(method_value) + 'temp.png', dst_total, 0)
        dst_total = None

        gdal.DEMProcessing(settings.MEDIA_ROOT + '/latvian_case/total' + 'annal' + str(annal_value) + 'fulu' + str(fulu_value) + 'fuve' + str(fuve_value) + 'mytr' + str(mytr_value) + 'sust' + str(sust_value) + 'method' + str(method_value) + '.png', ds2to, "color-relief",  colorFilename=settings.MEDIA_ROOT + '/latvian_case/color.txt', options="-alpha -of PNG")
        ds2to = None

        t = form.save(commit=False)

        t.url_sust = '/latvian_case/sust' + str(sust_value) + '.png'
        t.url_mytr = '/latvian_case/mytr' + str(mytr_value) + '.png'
        t.url_fuve = '/latvian_case/fuve' + str(fuve_value) + '.png'
        t.url_fulu = '/latvian_case/fulu' + str(fulu_value) + '.png'
        t.url_annal = '/latvian_case/annal' + str(annal_value) + '.png'
        t.url_result = '/latvian_case/total' + 'annal' + str(annal_value) + 'fulu' + str(fulu_value) + 'fuve' + str(fuve_value) + 'mytr' + str(mytr_value) + 'sust' + str(sust_value) + 'method' + str(method_value) + '.png'
        t.minx = minx
        t.maxx = maxx
        t.miny = miny
        t.maxy = maxy
        t.save()
        form = LatvianCaseForm()

    context = {
        'form': form,
    }
    return render(request, "umap/latvian_case.html", context)
